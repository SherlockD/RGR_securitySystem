...
namespace RGR_securitySystem.Crypt.AES
{
    public class AES
    {
        ...
        private readonly byte[] _quickXorTable = new byte[8] { 0x00, 0x1b, 0x36, 0x2d, 0x6c, 0x77, 0x5a, 0x41 };
        private byte[] _inverseMixColumnMatrixElementTable = new byte[] { 0x0B, 0x0D, 0x09, 0x0E, 0x0B, 0x0D, 0x09 };

        private readonly byte[] _roundCoefficient = new byte[ROUND_NO] {
            0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36 };
        
        #region ConstantBoxes    
        ...
        { //round no starts from 0, ends at 9
            if (roundno < 0 || roundno > ROUND_NO - 1)
                return;
            byte buf = keyword[0];
            keyword[0] = (byte)(sBox[keyword[1]] ^ _roundCoefficient[roundno]);
        ...
        private byte GaloisDefaultMult(byte val, byte mult) 
        {
            ...
            if ((mult & 0x02) > 0)
                buf ^= val << 1;
            byte xorval = _quickXorTable[buf >> 8];
        ...
        private void InverseMixColumn(ref byte[] text)
        {
            ...
            for (int i = 0; i < 4; ++i)
            {
                ...
                for (int j = 0; j < 4; ++j)
                {
                    p2 = 3 - j;
                    temp[j] =
                        (byte) (GaloisDefaultMult(text[p], _inverseMixColumnMatrixElementTable[p2]) ^ GaloisDefaultMult(
                                                                                                        text[p + 1],
                                                                                                        _inverseMixColumnMatrixElementTable
                                                                                                            [p2 + 1])
                                                                                                    ^ GaloisDefaultMult(
                                                                                                        text[p + 2],
                                                                                                        _inverseMixColumnMatrixElementTable
        ...
        public byte[] Encrypt(string text, ref byte[] ciphertext)
        {
            return Encrypt(Encoding.UTF8.GetBytes(text), ref ciphertext);
        }
        
        public byte[] Encrypt(byte[] plaintext, ref byte[] ciphertext)
        {
        ...
        public byte[] Decrypt(byte[] ciphertext, ref byte[] plaintext)
        {
